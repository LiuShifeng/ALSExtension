...
import wti.st.ALSExtension.ml.recommendation.{ALS => NewALS}
import wti.st.ALSExtension.mllib.recommendation
...
class ALS private (
   ...
   */
  @Since("1.3.0")
  def run(ratings: JavaRDD[recommendation.Rating]): MatrixFactorizationModel = run(ratings.rdd)
}
...
object ALS {
  ...
  @Since("0.9.1")
  def train(
             ratings: RDD[recommendation.Rating],
             rank: Int,
             iterations: Int,
             lambda: Double,
             blocks: Int,
   ...
   */
  @Since("0.8.0")
  def train(ratings: RDD[recommendation.Rating], rank: Int, iterations: Int, lambda: Double)
  : MatrixFactorizationModel = {
   ...
   */
  @Since("0.8.0")
  def train(ratings: RDD[recommendation.Rating], rank: Int, iterations: Int)
  : MatrixFactorizationModel = {
   ...
   */
  @Since("0.8.1")
  def trainImplicit(ratings: RDD[recommendation.Rating], rank: Int, iterations: Int, lambda: Double, alpha: Double)
  : MatrixFactorizationModel = {
   ...
   */
  @Since("0.8.1")
  def trainImplicit(ratings: RDD[recommendation.Rating], rank: Int, iterations: Int)
  : MatrixFactorizationModel = {
