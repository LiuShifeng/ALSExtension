 ...
 */
private[recommendation] trait PMFModelParams extends Params with HasPredictionCol {
  /**
 ...
 */
private[recommendation] trait PMFParams extends PMFModelParams with HasMaxIter with HasRegParam
 ...
 * Model fitted by ALS.
 */
class PMFModel private[ml] (
                             override val parent: PMF,
                             override val fittingParamMap: ParamMap,
                             rank: Int,
                             userFactors: RDD[(Int, Array[Float])],
                             itemFactors: RDD[(Int, Array[Float])])
  extends Model[PMFModel] with PMFParams {
 ...
 */
@Experimental
class PMF(override val uid: String) extends Estimator[PMFModel] with PMFParams {

  import org.apache.spark.ml.recommendation.PMF.Rating
  ...
  override def fit(dataset: DataFrame, paramMap: ParamMap): PMFModel = {
    val map = this.paramMap ++ paramMap
    val ratings = dataset
    ...
    val (userFactors, itemFactors) = PMF.train(ratings, rank = map(rank),
    ...
    val model = new PMFModel(this, map, map(rank), userFactors, itemFactors)
...
object PMF extends Logging {
  ...
  def train[ID: ClassTag]( // scalastyle:ignore
    ...
    val userPart = new PMFPartitioner(numUserBlocks)
    val itemPart = new PMFPartitioner(numItemBlocks)
  ...
  private def makeBlocks[ID: ClassTag](
    ...
    }.groupByKey(new PMFPartitioner(srcPart.numPartitions))
      .mapValues { iter =>
      val builder =
  ...
  private def computeFactors[ID](
    ...
    val minParam = if(implicitPrefs) math.min(math.min(regParam,1.0),alpha) else math.min(regParam,1.0)
    val maxParam = if(implicitPrefs) math.max(math.max(regParam,1.0),alpha) else math.max(regParam,1.0)
    ...
    val merged = srcOut.groupByKey(new PMFPartitioner(dstInBlocks.partitions.length))
        ...
        while (j < dstIds.length) {
          ...
          while (i < srcPtrs(j + 1)) {
            ...
            if (implicitPrefs) {
              ...
              val c1 = if(rating == 0.0.toFloat) magnify * alpha else magnify
   ...
   */
  private[recommendation] type PMFPartitioner = org.apache.spark.HashPartitioner
}
