 ...
 */
private[recommendation] trait SoRecModelParams extends Params with HasPredictionCol {
  /**
...
class SoRec(override val uid: String) extends Estimator[SoRecModel] with SoRecParams {
  ...
  /** @group setParam */
  def setBelta(value: Double): this.type = set(belta, value)
...
object SoRec extends Logging {
  ...
  def train[ID: ClassTag]( // scalastyle:ignore
    ...
    val userPart = new SoRecPartitioner(numUserBlocks)
    val itemPart = new SoRecPartitioner(numItemBlocks)
    ...
    if (implicitPrefs) {
      for (iter <- 1 to maxIter) {
        ...
        itemFactors = computeFactors(userFactors, userOutBlocks, itemInBlocks, rank, regParam,
          userLocalIndexEncoder, implicitPrefs, alpha, belta, items, gamma, solver)
        ...
        userFactors = computeFactors(itemFactors, itemOutBlocks, userInBlocks, rank, regParam,
          itemLocalIndexEncoder, implicitPrefs, alpha, belta, items, gamma, solver)
        if (shouldCheckpoint(iter)) {
      ...
      for (iter <- 0 until maxIter) {
        itemFactors = computeFactors(userFactors, userOutBlocks, itemInBlocks, rank, regParam,
          userLocalIndexEncoder, items = items, gamma = gamma, solver = solver)
        if (shouldCheckpoint(iter)) {
        ...
        userFactors = computeFactors(itemFactors, itemOutBlocks, userInBlocks, rank, regParam,
          itemLocalIndexEncoder, items = items, gamma = gamma, solver = solver)
      }
    }
  ...
  private def makeBlocks[ID: ClassTag](
    ...
    }.groupByKey(new SoRecPartitioner(srcPart.numPartitions))
      .mapValues { iter =>
      val builder =
   ...
   * @param belta the belta constant in the implicit preference formulation for social influence
   * @param items the items number in the combined matrix
   * @param gamma the gamma constant for social influence
  ...
  private def computeFactors[ID](
    ...
    val minParam = if(implicitPrefs) math.min(math.min(math.min(regParam,gamma),1.0),math.min(alpha,belta)) else math.min(math.min(regParam,gamma),1.0)
    val maxParam = if(implicitPrefs) math.max(math.max(math.max(regParam,gamma),1.0),math.max(alpha,belta)) else math.max(math.max(regParam,gamma),1.0)
    val magnify = maxParam/minParam
    ...
    val merged = srcOut.groupByKey(new SoRecPartitioner(dstInBlocks.partitions.length))
        ...
        while (j < dstIds.length) {
          ...
          while (i < srcPtrs(j + 1)) {
            ...
            if (implicitPrefs) {
              ...
              val c1 = if(i < items){if(rating == 0.0.toFloat) magnify*alpha else magnify}else{if(rating == 0.0.toFloat) magnify * belta * gamma * magnify else gamma * magnify}
          ...
          dstFactors(j) = solver.solve(ls, numExplicits * magnify * regParam)
          j += 1
        }
        dstFactors
    }
   ...
   */
  private[recommendation] type SoRecPartitioner = org.apache.spark.HashPartitioner
}
