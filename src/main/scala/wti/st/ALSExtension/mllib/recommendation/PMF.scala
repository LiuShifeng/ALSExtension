...
import wti.st.ALSExtension.ml.recommendation.{PMF => NewPMF}
...
class PMF private (
  ...
  def run(ratings: RDD[Rating]): MatrixFactorizationModel = {
    ...
    val (floatUserFactors, floatProdFactors) = NewPMF.train[Int](
      ratings = ratings.map(r => NewPMF.Rating(r.user, r.product, r.rating.toFloat)),
      rank = rank,
...
object PMF {
  ...
  def train(
    ...
    new PMF(blocks, blocks, rank, iterations, lambda, false, 1.0, seed).run(ratings)
  }
  ...
  def train(
    ...
    new PMF(blocks, blocks, rank, iterations, lambda, false, 1.0).run(ratings)
  }
  ...
  def trainImplicit(
    ...
    new PMF(blocks, blocks, rank, iterations, lambda, true, alpha, seed).run(ratings)
  }
  ...
  def trainImplicit(
    ...
    new PMF(blocks, blocks, rank, iterations, lambda, true, alpha).run(ratings)
  }
