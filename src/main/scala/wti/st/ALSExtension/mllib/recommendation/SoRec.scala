...
class SoRec private (
  ...
  def run(ratings: RDD[Rating]): MatrixFactorizationModel = {
    ...
    val (floatUserFactors, floatProdFactors) = NewSoRec.train[Int](
      ratings = ratings.map(r => NewSoRec.Rating(r.user, r.product, r.rating.toFloat)),
      rank = rank,
...
object SoRec {
  ...
  def train(
    ...
    new SoRec(blocks, blocks, rank, iterations, items, lambda, false, 1.0,1.0,gamma, seed).run(ratings)
  }
  ...
  def trainImplicit(
    ...
    new SoRec(blocks, blocks, rank, iterations, items, lambda, true, alpha, belta, gamma,seed).run(ratings)
  }
  ...
  def trainImplicit(
    ...
    new SoRec(blocks, blocks, rank, iterations, items, lambda, true, alpha, belta, gamma).run(ratings)
  }
